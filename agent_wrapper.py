...
def show_progress(rewards_batch: torch.Tensor, log: list,
    ...
    means = [m[0] for m in log]
    thresholds = [t[1] for t in log]

    plt.plot(np.arange(1, len(log) + 1), means, label='Средняя награда')
    plt.plot(np.arange(1, len(log) + 1), thresholds, label=f'{percentile:}-й перцентиль')
    ...
    plt.subplot(1, 2, 2)
    # your code here
    rewards_np = rewards_batch.cpu().numpy()
    
    plt.hist(rewards_np, bins=20, range=reward_range, color='pink', alpha=0.84)
    plt.axvline(x = threshold, color='red', label=f'{percentile}-й перцентиль = {threshold:.3f}')
    plt.axvline(x = mean_reward, color='blue', label=f'Среднее = {mean_reward:.3f}')
...
class GYMAgentWrapper():
    ...
    def select_best_session(
        ...
        reward_threshold = torch.quantile(rewards_batch, percentile / 100).item() # порог наград, расчитывается с помощью перцентиля
        ...
        # при strict == True, отобранны должны быть только states и actions с вознограждением > перцентиля - отобраНы, вознАграждение (сори за токсичность)
        # при strict == False, отобранны должны быть только states и actions с вознограждением >= перцентиля - отобраНы, вознАграждение (сори за токсичность x2)
        ##########################
        # your code here
